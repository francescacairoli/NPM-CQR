...
class DetermCQR():
	...
	def __init__(self, Xc, Yc, trained_qr_model, quantiles = [0.05, 0.95]):
		self.Xc = Xc 
		self.Yc = Yc
	...
	def get_calibr_nonconformity_scores(self, y, pred_interval, sorting = True):
		...
		for i in range(n):
			...
			ncm[i] = max(pred_interval[i,0]-y[i], y[i]-pred_interval[i,-1]) # pred_interval[i,0] = q_lo(x), pred_interval[i,1] = q_hi(x)
	...
	def plot_results(self, y_test, test_pred_interval, title_string, plot_path):
		...
		plt.scatter(xline, y_test[:n_points_to_plot], c='b', s=1, alpha = 0.1)
