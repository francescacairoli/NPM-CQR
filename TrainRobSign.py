...
class TrainRobSign():
	...
	def __init__(self, model_name, dataset, idx = None, cal_hist_size = 50, test_hist_size = 2000, quantiles = [0.05, 0.95], opt = "Adam", n_hidden = 50, scheduler_flag = False):
	...
	def train(self, n_epochs, batch_size, lr):
		...
		C_val = np.repeat(self.dataset.C_cal, self.cal_hist_size, axis = 0)[:(self.valid_set_dim*self.cal_hist_size)]
		...
		for epoch in range(n_epochs):
			...
			for i in range(bat_per_epo):
				...
				#print("xxxxxxxxxx", hypothesis.size(), Lt.size(), Xt.size())
